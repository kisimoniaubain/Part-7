...
import Notification from './components/Notification' // 1. Import the new Notification component
import { useNotificationDispatch } from './NotificationContext' // 2. Import the context dispatcher
...
const App = () => {
  ...
  const dispatch = useNotificationDispatch() // 3. Initialize the global context dispatcher
  ...
  const clearUser = useBlogStore((state) => state.clearUser) 
  ...
  const handleLogin = async (event) => {
    ...
    try {
      ...
      dispatch({ type: 'SET_NOTIFICATION', payload: `Welcome back, ${targetUser.username}!` })
  ...
  const handleCreate = async (newAnecdote) => {
    try {
      ...
      // 4. Dispatch a success action to our reducer context!
      dispatch({ type: 'SET_NOTIFICATION', payload: `Anecdote '${newAnecdote.content}' successfully created!` })
      
      // Automatically clear it after 5 seconds
      setTimeout(() => {
  ...
  return (
    <div style={{ fontFamily: 'sans-serif' }}>
      ...
      {/* 5. Place the notification banner right above your pages content views */}
      <Notification />
